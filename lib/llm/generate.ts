...
- 제공된 선행기술이 적거나 없을 때(예: "검색된 선행기술 없음")는 종래기술을 단정적으로 서술하지 말고 일반적 기술 배경으로 폭넓게 기술하세요. 단정적 신규성·진보성 주장은 피하고, 디브리프에 없는 구체 수치·실험값·정량 효과를 새로 지어내지 마세요. 수치·범위·재료사양이 필요하면 디브리프에서만 가져오고, 없으면 "일 실시예에서/예시적으로 …일 수 있다"처럼 hedge하여 예시임을 명시하세요(없는 값을 단정 금지).
...
실시가능요건(KIPO 특허법 §42 — 발명의 설명/구체적인 내용/실시예에서 특히 중요):
- 본문은 통상의 기술자가 과도한 시행착오 없이 발명을 실시할 수 있을 만큼 구성·작용·효과·실시예를 구체적으로 기술해야 합니다. 단순 효과 나열이 아니라 "어떻게 그 효과가 달성되는지"의 작용원리를 설명하세요.
- 「발명을 실시하기 위한 구체적인 내용」·「실시예」에서는 가능한 한 다음을 포함하세요: (a) 각 구성요소의 구조·배치·결합관계(도면부호 사용), (b) 동작 흐름(입력 → 처리 → 출력)을 단계적으로, (c) 핵심 파라미터의 예시적 범위·조건(디브리프에 있으면 그 값을, 없으면 "일 실시예에서 … 정도일 수 있다"로 예시임을 밝힘 — 없는 수치를 단정하지 말 것), (d) 가능하면 best mode(발명자가 생각하는 최적 실시형태), (e) 대체·변형 실시예로 권리범위가 특정 구현에 한정되지 않음을 보여주기.
- 「과제의 해결 수단」은 독립항 범위와 정합하는 핵심 구성요소를 빠짐없이 기술하여, 청구항 용어가 명세서에 뒷받침(antecedent basis)되도록 하세요(기재불비 방지). 단, 이는 명세서 본문 기술일 뿐이며 실제 청구항 텍스트는 작성하지 않습니다.
- 구체성을 높이되 grounding을 위반하지 마세요: 구체화는 디브리프·선행기술[1]~[N]에 근거해야 하며, 근거 없는 수치·실험값·고유사양을 지어내 채우지 마세요(부족하면 hedge).
...
차별점: ${input.differentiation?.length ? input.differentiation.join(' / ') : '없음'}${editBlock}${exemplarBlock}`
  ...
  return runTool<{ text: string; cited_ref_ns: number[] }>({
    ...
    // Headroom for the deeper enablement-grade body (구성·작용·실시예·대체실시예); 2500
    // truncated the richer 「발명을 실시하기 위한 구체적인 내용」/「실시예」, and runTool turns a
    // mid-tool_use max_tokens stop into a hard error. Per-section (one section per call).
    maxTokens: 3800,
    onUsage,
  })
}
